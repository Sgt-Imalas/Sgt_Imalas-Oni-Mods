...
namespace RonivansLegacy_ChemicalProcessing.Content.Scripts
{
	class SettingsCache_Patches
	{
		...
		public class SettingsCache_LoadBiome_Patch
		{
			...
			[HarmonyPrepare]
			public static bool Prepare() => Config.Instance.WorldgenElementInjection;
			...
			public static void Postfix(string longName, List<YamlIO.Error> errors, ref bool __state)
			{
				if (!__state)
					return;
				...
				var biomeSettings = SettingsCache.biomeSettingsCache[path];

				BiomeElementInjections.Inject(longName, biomeSettings);
			}
		}
	}
}
