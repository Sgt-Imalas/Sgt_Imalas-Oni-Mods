...
namespace MinnowStoryTrait.Patches
{
	class StoryContentPanel_Patches
	{
		...
		/// <summary>			
		/// disable custom story trait if it got rolled randomly on a cluster that already has it			
		/// </summary>
		...
		public class StoryContentPanel_SelectRandomStories_Patch
		{
			public static void Postfix(StoryContentPanel __instance, bool useBias)
			{
				...
				DisableMinnowOnAquaticWorld(clusterId, __instance);
			}
		}
		...
		public class CustomGameSettings_SetQualitySetting_Patch
		{
			public static void Postfix(CustomGameSettings __instance, SettingConfig config, string value)
			{
				...
				DisableMinnowOnAquaticWorld(value);
			}
		}
		...
		{			
			...
			{
				if (storyId != CGMWorldGenUtils.CGM_Minnow_StoryTrait)
					return true;
				...
				if (CGMWorldGenUtils.HasMinnowInCluster(clusterId))
				{
			...
			public static void Postfix(bool __state)
			{
				if (__state)
				{
					var dlc5State = CustomGameSettings.Instance.GetCurrentMixingSettingLevel(CustomMixingSettingsConfigs.DLC5Mixing);
					if (dlc5State.id == DlcMixingSettingConfig.DisabledLevelId)
					{
						CustomGameSettings.Instance.SetMixingSetting(CustomMixingSettingsConfigs.DLC5Mixing, DlcMixingSettingConfig.EnabledLevelId);
					}
				}
			}
		}
		...
		public class CustomGameSettings_SetMixingSetting_Patch
		{
			public static void Postfix(CustomGameSettings __instance, SettingConfig config, string value)
			{
				if (config != CustomMixingSettingsConfigs.DLC5Mixing)
					return;
				DisableMinnowOnAquaticWorld(value);
			}
		}

		public static void DisableMinnowOnAquaticWorld(string clusterId, StoryContentPanel __instance = null)
		{
			var settings = CustomGameSettings.Instance;
			if (!settings.StorySettings.ContainsKey(CGMWorldGenUtils.CGM_Minnow_StoryTrait))
			{
				return;
			}

			if (CGMWorldGenUtils.HasMinnowInCluster(clusterId) || settings.GetCurrentMixingSettingLevel(CustomMixingSettingsConfigs.DLC5Mixing).id == DlcMixingSettingConfig.DisabledLevelId)
			{
				var story = settings.StorySettings[CGMWorldGenUtils.CGM_Minnow_StoryTrait];
				...
				if (__instance == null)
				{
					...
					var currentlyDisabledTraits = settings.StorySettings
							.Where(pair => settings.GetCurrentStoryTraitSetting(pair.Key).id == "Disabled" && pair.Key != CGMWorldGenUtils.CGM_Minnow_StoryTrait);
				...
				{
					__instance.SetStoryState(CGMWorldGenUtils.CGM_Minnow_StoryTrait, StoryContentPanel.StoryState.Forbidden);

					var currentlyDisabledTraits = __instance.storyStates
						.Where(pair => pair.Value == StoryContentPanel.StoryState.Forbidden && pair.Key != CGMWorldGenUtils.CGM_Minnow_StoryTrait);
