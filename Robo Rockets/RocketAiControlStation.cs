...
namespace KnastoronOniMods
{
    class RocketAiControlStation : KMonoBehaviour
    {
        ...
        private void MakeNewScoutBot()
        {
            ...
            {
                GameObject go = GameUtil.KInstantiate(Assets.GetPrefab((Tag)"ScoutRover"), Grid.CellToPos(Grid.CellRight(Grid.PosToCell(this.gameObject))), Grid.SceneLayer.Creatures);
                go.SetActive(true);
