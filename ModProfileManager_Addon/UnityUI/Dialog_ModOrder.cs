...
namespace ModProfileManager_Addon.UnityUI
{
    internal class Dialog_ModOrder
    {
        ...
        public void InsertMod(int TargetPosition, bool atEnd = false, bool atStart = false)
        {
            if (ModAssets.SelectedModPack.ModList.TryGetModListEntry(ModAssets.SelectedModPack.Path, out var mods))
            {

                var label = mods[CurrentModIndex];
                ...
                {
                    mods.Insert(TargetPosition, label);
                }
                ModAssets.SelectedModPack.ModList.WriteModlistToFile();
                ModsPresetScreen.Instance?.RebuildModsScreen();
            }
        }
