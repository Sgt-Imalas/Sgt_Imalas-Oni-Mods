...
{
	internal class RocketSimpleInfoPanel_Patches
	{
		[HarmonyPatch(typeof(RocketSimpleInfoPanel), nameof(RocketSimpleInfoPanel.Refresh))]
		public static class OptimizedRewriteForInfoPanel
		{
			...
			///robo pilot handling
			static float RemainingDatabanks = -1, DatabankRange = -1, DatabankRangePREV = 0f;
			static int DatabankConsumption = -1;
			static bool HasPassengerModule, HasRobotModule;
			...
			public static bool Prefix(RocketSimpleInfoPanel __instance, CollapsibleDetailContentPanel rocketStatusContainer, GameObject selectedTarget)
			{
				...
				HasPassengerModule = false;
				HasRobotModule = false;
				RemainingDatabanks = -1;
				DatabankConsumption = -1;
				DatabankRange = -1;
				...
				if (craftModuleInterface != null)
				{
					...
					foreach (var module in craftModuleInterface.clusterModules)
					{
						...
						if (moduleGet.TryGetComponent<PassengerRocketModule>(out var controlModule))
						{
							HasPassengerModule = true;
						}
						else if (moduleGet.TryGetComponent<RoboPilotModule>(out var robotModule))
						{
							HasRobotModule = true;
							RemainingDatabanks = robotModule.GetDataBanksStored();
							DatabankConsumption = robotModule.dataBankConsumption;
							DatabankRange = robotModule.GetDataBankRange();
						}
					...
					bool is_sole_robo_pilot = HasRobotModule && !HasPassengerModule;
					...
					if (!FuelSorted)
					{
						foreach (var module in craftModuleInterface.clusterModules)
						{
							...
							if (moduleGet.TryGetComponent<IFuelTank>(out var fueltank))
							{
								...
								FuelRemaining += ModIntegration_Patches.Hydrocarbon_Rocket_Engines.GetEffectiveFuelTankCapacity(fueltank.Storage, FuelTag);
					...
					if ((!Mathf.Approximately(FuelRemaining, FuelRemainingPREVIOUS))
						|| (RequiresOxidizer ? (!Mathf.Approximately(OxidizerRemaining, OxidizerRemainingPREVIOUS)) : false)
						|| (!Mathf.Approximately(FuelPerHexEngine, FuelPerHexEnginePREVIOUS))
						|| !Mathf.Approximately(DatabankRange, DatabankRangePREV))
					{
						redrawPanel = true;
						if (targetEngine != null)
						{
							...
							if (is_sole_robo_pilot)
								RangeRemainingToolTipSTRING = string.Concat(RangeRemainingToolTipSTRING, "\n", string.Format(global::STRINGS.UI.CLUSTERMAP.ROCKETS.RANGE.ROBO_PILOTED_TOOLTIP, DatabankConsumption, RemainingDatabanks));
							...
							if (is_sole_robo_pilot)
							{
								rangeRemaining = Mathf.Min(rangeRemaining, DatabankRange);
							}
						...
						DatabankRangePREV = DatabankRange;
					}

					///Rocket speed
					if (redrawPanel || !Mathf.Approximately(RocketBurden, RocketBurdenPREVIOUS) || !Mathf.Approximately(RocketEnginePower, RocketEnginePowerPREVIOUS))
					{
						redrawPanel = true;

						bool dupe_piloted = clusterCraft.AutoPilotMultiplier > 0.5f;
						bool robo_piloted = RemainingDatabanks > 0;

						float baseSpeed = RocketEnginePower / RocketBurden;
						Speed = baseSpeed * clusterCraft.PilotSkillMultiplier;

						if (robo_piloted && dupe_piloted)
						{
							Speed *= 1.5f;
						}
						else if (!dupe_piloted && HasPassengerModule)
						{
							Speed *= 0.5f;
						}
						else if (!robo_piloted && !HasPassengerModule)
						{
							Speed = 0;
						}
						...
						{
							controlType = "\n    • " + global::STRINGS.UI.CLUSTERMAP.ROCKETS.SPEED.SUPERPILOTED_SPEED_TOOLTIP.Replace("{speed_boost}", GameUtil.GetFormattedPercent(50f));
						}
						else if (HasPassengerModule && !dupe_piloted)
						{
							controlType = "\n    • " + global::STRINGS.UI.CLUSTERMAP.ROCKETS.SPEED.UNPILOTED_SPEED_TOOLTIP.Replace("{speed_boost}", GameUtil.GetFormattedPercent(50f));
						}
						else if (robo_piloted)
						{
							controlType = "\n    • " + global::STRINGS.UI.CLUSTERMAP.ROCKETS.SPEED.ROBO_PILOT_ONLY_SPEED_TOOLTIP;
						}
						else if (HasRobotModule && !HasPassengerModule)
						{
							controlType = "\n    • " + global::STRINGS.UI.CLUSTERMAP.ROCKETS.SPEED.DEAD_ROBO_PILOT_ONLY_SPEED_TOOLTIP;
						}
