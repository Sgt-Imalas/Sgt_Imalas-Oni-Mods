	...
	public class AttachmentPointTagNameFix : PForwardedComponent
	{
		...
		public override void Initialize(Harmony plibInstance)
		{
			try
			{
				...
				plibInstance.Patch(targetMethod, postfix: new(postfix, Priority.High));
				plibInstance.Patch(targetMethod2, postfix: new(postfix, Priority.High));
		...
		public static void Postfix(BuildingDef __instance, ref string fail_reason, ref bool __result)
		{
			...
			if (!properAttachmentPointTagString.Contains(missing))
			{
		...
		const string missing = "MISSING.STRINGS.";
	}
}
