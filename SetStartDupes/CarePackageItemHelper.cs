...
namespace SetStartDupes
{
	internal class CarePackageItemHelper
	{
		...
		{
			GameObject prefab = Assets.TryGetPrefab((Tag)CarePackageID);
			if (prefab == null)
			{
		...
		public static string GetSpawnableQuantity(string CarePackageID, float CarePackageQuantity)
		{
			if (ElementLoader.GetElement(CarePackageID.ToTag()) != null)
				return string.Format((string)global::STRINGS.UI.IMMIGRANTSCREEN.CARE_PACKAGE_ELEMENT_COUNT, Assets.TryGetPrefab((Tag)CarePackageID).GetProperName(), GameUtil.GetFormattedMass(CarePackageQuantity));

			var info = EdiblesManager.GetFoodInfo(CarePackageID);

			return info != null && info.CaloriesPerUnit > 0
				? string.Format((string)global::STRINGS.UI.IMMIGRANTSCREEN.CARE_PACKAGE_ELEMENT_COUNT, Assets.TryGetPrefab((Tag)CarePackageID).GetProperName(), GameUtil.GetFormattedCaloriesForItem((Tag)CarePackageID, CarePackageQuantity))
				: string.Format((string)global::STRINGS.UI.IMMIGRANTSCREEN.CARE_PACKAGE_ELEMENT_COUNT, Assets.TryGetPrefab((Tag)CarePackageID).GetProperName(), CarePackageQuantity.ToString());
		}
